...
class ProfileRepoImpl extends ProfileRepo {
  ...
  Future<Either<String, Null>> updateGymInfo({
    ...
    try {
      final formData = FormData.fromMap({
        ...
        ApiKeys.photos: gymGalary == null
            ? null
            : await convertFilesToMultipart(gymGalary),
      });
      await api.put(
  ...
  @override
  Future<Either<String, Null>> driverUpdateProfile({
    required String id,
    required String fullName,
    required String phone,
    ...
    try {
      final formData = FormData.fromMap({
        ...
        ApiKeys.vehicleModel: vehicleModel,
        ApiKeys.vehicleNumber: vehicleNumber,
        ApiKeys.drivingExperienceYears: drivingExperienceYears,
        ...
        if (carImages != null)
          ApiKeys.carImages: await convertFilesToMultipart(carImages),
      });
      await api.put(
  ...
  Future<Either<String, Null>> technicianUpdateProfile({
    ...
    try {
      final formData = FormData.fromMap({
        ApiKeys.profilePhotoCap: await convertImageToMultipart(photo),
        ApiKeys.documents: technicantDocuments == null
            ? null
            : await convertPlatformFilesToMultipart(technicantDocuments),
      });

      await api.put(
        ApiEndPoints.technicantUpdateProfile,
        queryParameters: {
          ApiKeys.id: id,
          ApiKeys.fullName: fullName,
          ApiKeys.phone: phone,
